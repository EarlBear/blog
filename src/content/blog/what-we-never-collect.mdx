...
rule is untouched. What we allowed is syncing a *scrubbed* transcript (every string redacted)
alongside the derived record, so there's a cross-machine corpus to render from. That's not the
central-raw store this post rules out: the file is scrubbed at the edge and must clear a
gitleaks-required, fail-closed gate before it goes anywhere. The honest wrinkle is that a
scrubbed *full* transcript is a weaker (regex) guarantee than the derived record's structural
one — which is exactly why that new path requires the second detector rather than trusting our
own redaction alone. The negative choice at the center of this post still holds; we were careful
about the one line we relaxed. The full reasoning is in
