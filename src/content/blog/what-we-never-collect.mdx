...
<FlowDiagram
  title="Raw stays, only the shadow travels"
  shape="pipeline"
  legend
  desc="On each machine, raw JSONL is analyzed at the edge into a derived record. Only that derived record crosses to the shared database. The raw file has no route off the machine — the boundary is structural, not a policy."
  nodes={[
    { id: 'raw', label: 'Raw JSONL', kind: 'store', detail: 'Full prompts, file contents, the occasional pasted secret. Stays in each machine’s ~/.claude/projects/ — never collected, never moved.' },
    { id: 'edge', label: 'Analyze at edge', kind: 'edge', detail: 'A stdlib-only pure function runs on each machine, deriving metrics and two short redacted excerpts from an allowlist of safe fields. No central analyzer, because that would require shipping it the raw file.' },
    { id: 'derived', label: 'Derived record', detail: 'Token counts, tool names, timings, redacted excerpts. The only thing that travels — the safe shadow of the raw file.' },
    { id: 'db', label: 'Shared database', kind: 'external', detail: 'The collection point, holding no raw content. Aggregation happens here across machines, from data scrubbed before it left the edge. Locally, a Postgres container stands in for it.' },
  ]}
  edges={[
    { from: 'raw', to: 'edge' },
    { from: 'edge', to: 'derived' },
    { from: 'derived', to: 'db', label: 'crosses the boundary' },
  ]}
/>
...
## Since then: "never sync raw" ≠ "never sync anything"

We later added a way for several machines to pool their telemetry through a shared folder, and
it's worth being precise about what that did and didn't change. We *never* sync raw JSONL — that
rule is untouched. What we allowed is syncing a redacted **event log** alongside the derived
record: an append-log of what the agent did (tool calls, subagent runs, a metrics snapshot),
built from the *same safe-field allowlist* as the derived record. We briefly considered shipping
a scrubbed copy of the *whole transcript* instead — richer, but it would have made a regex pass
the load-bearing defense against arbitrary personal data, so we didn't. Neither artifact is the
central-raw store this post rules out: both are built at the edge from named safe fields and must
clear a gitleaks-required, fail-closed gate before they go anywhere. The negative choice at the
center of this post held — and choosing the event log over the transcript was us holding it a
second time. The full reasoning is in
