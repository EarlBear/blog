...
- [x] Create `new-diagram-kind` skill (thinking process for adding a new diagram kind: survey the catalog first, match the house engine, prove it, keep the catalog living) + catalog-upkeep hook (check-diagram-catalog.py, npm run catalog-check, make check, PostToolUse) that flags a diagram component or FlowDiagram shape missing from catalog.md; wired into CLAUDE.md @done(2026-07-05T00:00:00Z) #git:pending
