...
- [x] Frontmatter-driven visual-expectation check: `expects: [comparison|flow|use-case|decision]` field in the zod schema; check-visual-expectations.py (npm run expects-check, make check, PostToolUse hook, advisory) warns when a declared visual's component is absent; declared expects:[comparison] on one-database; documented in new-post skill @done(2026-07-05T00:00:00Z) #git:pending
