...
- [x] Build FlowDiagram primitive (pipeline/loop/sequence): zero-dep build-time inline SVG, gradient nodes, edge/store/external node kinds, blocking overlap gate, draw-in animation, click-to-focus modal, auto-loop from back-edge @done(2026-07-04T00:00:00Z) #git:pending
- [x] Build "our extended Mermaid": zero-dep parser (mermaid-parse.ts) for the Mermaid flowchart subset → FlowDiagram; verified no pure-JS Mermaid renderer works here (getBBox on jsdom/svgdom fails on Node 20+22), so we parse not render — no browser, no dep @done(2026-07-04T00:00:00Z) #git:pending
- [x] Build Accordion foldable-options component (native details/summary, zero JS, verdict pills) for decision posts @done(2026-07-04T00:00:00Z) #git:pending
- [x] Enrich 4 proof posts (→.mdx): agentic-workflow (loop), mining-transcripts (pipeline w/ edge boundary), from-one-laptop (architecture), one-database-two-modes (accordion) @done(2026-07-04T00:00:00Z) #git:pending
- [x] Build enrich-post skill + living catalog.md (technique table + Mermaid decision rule) + wire into CLAUDE.md; fix new-post dangling dataviz ref; diagram-catalog feature doc @done(2026-07-04T00:00:00Z) #git:pending
