...
def check_post(path: Path, root: Path):
    ...
    # clear net-internal lean.
    if declared == "external" and (internal >= 3 or internal - external >= 3):
        warns.append(
            f"{rel}: marked audience: external but reads internal "
            f"(signals: {', '.join(sorted(set(reasons)))}). Confirm it's safe to "
            "publish publicly, or move it to audience: internal."
        )
    # Reads clearly external but marked internal → possibly over-restricted.
    elif declared == "internal" and external >= 2 and internal == 0:
        warns.append(
