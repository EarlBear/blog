...
| **data model / ER** | entities, their fields (PK/FK), and the relationships between them (cardinality) | `DataModel` — a **true ERD**: entity boxes (fields + PK/FK) on a deterministic grid, **connected by crow's-foot cardinality edges** (one · zero-or-one · many · zero-or-many) with labeled relations; a visually-hidden relationship list is the a11y/no-JS fallback | native spec **or** `mermaid` string: native `entities=[{id,label,fields:[{name,type,key:'pk'\|'fk'}]}]` + `relations=[{from,to,label,card}]`; OR `mermaid={\`erDiagram\n A \|\|--o{ B : has\n A { string id PK }\`}` (parsed by `parseMermaidER` — our extended Mermaid, ER edition). optional `columns` | `ecommerce-site-scanner-design`, `self-healing-storefront-design` |
| **requirements** | the per-actor capability contract of a design — what each actor MUST / CAN / must NOT do | `Requirements` (one row per actor; must/can/cannot columns) | native spec: `groups=[{actor,must:[],can:[],cannot:[]}]` | (design-post skeleton) |
| **personas / impact** | *whose* life the design touches and *how* — role → what changes for them / what they do with it | `Personas` (role → impact, one per line; optional `before`) | native spec: `personas=[{role,impact,before?}]` | (design-post skeleton) |
| **use cases (by actor)** | what a design POWERS, in three columns by actor (EarlBear · Agent · Customer); sits under a `UseCaseDiagram` | `UseCases` (3-column value grid) | native spec: `earlbear=[]`, `agent=[]`, `customer=[]` (or custom `columns`) | (design-post skeleton) |
